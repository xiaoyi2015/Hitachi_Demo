 ...
 */
public class ExpandableSwipeListViewTouchListener implements View.OnTouchListener {
    ...
    // Fixed properties
    private ExpandableSwipeListView swipeListView;
    ...
    //private List<ArrayList<Boolean>> opened = new ArrayList<ArrayList<Boolean>>();
    private List<Boolean> openedRight = new ArrayList<Boolean>();
    //private List<ArrayList<Boolean>> openedRight = new ArrayList<ArrayList<Boolean>>();
    private boolean listViewMoving;
    private List<Boolean> checked = new ArrayList<Boolean>();
    //private List<ArrayList<Boolean>> checked = new ArrayList<ArrayList<Boolean>>();
     ...
     */
    public ExpandableSwipeListViewTouchListener(ExpandableSwipeListView swipeListView, int swipeFrontView, int swipeBackView, int swipeGroupView) {
    ...
    /*public void resetItems() {
        if (swipeListView.getAdapter() != null) {
            int count = swipeListView.getExpandableListAdapter().getGroupCount();
            for (int i = 0; i <= count; i++) {
                if (i>=opened.size()){
                    opened.add(new ArrayList<Boolean>(){{ add(false); }});
                    openedRight.add(new ArrayList<Boolean>(){{ add(false); }});
                    checked.add(new ArrayList<Boolean>(){{ add(false); }});
                } else {
                    int count2 = swipeListView.getExpandableListAdapter().getChildrenCount(i);
                    for (int k = opened.get(i).size(); k <= count2; k++) {
                        opened.get(i).add(false);
                        openedRight.get(i).add(false);
    ...
    /**
     * Clears all states cached (Created by Cristiano)
     */
